...
* 登入時，在前端有用sha1加密，後端儲存的密碼是加密過的，所以在登入傳送去後端比對時 密碼也會是加密過的去比對。

* 註冊有用sha1加密，後端儲存的密碼是加密過的，所以在登入傳送去後端比對時 密碼也會是加密過的去比對。

* 老師測試帳號和密碼都是isu

* 學生測試帳號和密碼都是123456

* 此JavaScript有些是當初模板裡的JavaScript，尚未將沒有用到的功能刪除，另外所有在首頁用到的圖片都在img資料夾裡面

* 在所有頁面將Footer註解掉是因為Footer在css檔案裡面是寫死的，目前會影響頁面閱讀，所以註解

* 學生頁面的心得填寫有處理心得不能為空就送出，此功能(javascript)寫在credential_editor.html裡的166行到176行
